...
namespace html
{
    ...
    public:
        ...
        node(node &&d) noexcept
            : type_node(d.type_node), type_tag(d.type_tag), self_closing(d.self_closing), tag_name(std::move(d.tag_name)), content(std::move(d.content)), parent(nullptr), bogus_comment(d.bogus_comment), children(std::move(d.children)), attributes(std::move(d.attributes)), index(0), node_count(d.node_count) {}
