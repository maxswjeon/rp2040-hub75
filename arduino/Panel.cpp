...
int Panel::printChar(int32_t x, int32_t y, uint32_t c, uint8_t color)
{
    ...
    else if (c >= 0xAC00 && c <= 0xD7A3)
    {
        ...
        else
        {
            ...
            for (uint32_t i = 0; i < 16 * 16; ++i)
            {
                uint8_t data = _font_ko[cho * 32 + i / 8] | _font_ko[jung * 32 + i / 8] | _font_ko[jong * 32 + i / 8];
                uint8_t flag = (data & (1 << (7 - (i % 8)))) != 0 ? 1 : 0;
                setPixel(x + (i % 16), y + (i / 16), flag * color);
            }
        }
        return 16;
...
void Panel::printString(int32_t x, int32_t y, const wchar_t *str, uint8_t color)
{
    uint32_t cur_x = x;
    uint32_t cur_y = y;
