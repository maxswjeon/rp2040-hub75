...
int main()
{
    ...
    uint64_t color_update = 0;
    int colors[32];

    while (true)
    {
        if (time_us_64() - color_update > 1 * 1000 * 1000)
        {
            for (int i = 0; i < 32; ++i)
            {
                colors[i] = get_rand_32() % 8;
            }
            color_update = time_us_64();
        }

        for (int i = 0; i < 32; ++i)
        {
            drawLine(i, (colors[i] & 0b100) >> 2, (colors[i] & 0b010) >> 1, (colors[i] & 0b001) >> 0);
        }
    }
}